#!/bin/bash
#SBATCH --job-name=anneal-rotation-angle
...
export RECIPE='rotation'
export ATTACKITER='10'
export ATTACKOPTIM='PGD'
export ROTATIONANGLE='1.0'
...
--poison_path ${POISON_DATASET_DIR}/${RECIPE}_${MODEL_NAME}_optim=${ATTACKOPTIM}_iter=${ATTACKITER}_slurmid=${SLURM_JOBID} \
--attackoptim ${ATTACKOPTIM} --attackiter $ATTACKITER --rotation_angle_factor $ROTATIONANGLE
...
python poison_evaluation/eval_transferability.py ${POISON_DATASET_DIR}/${RECIPE}_${MODEL_NAME}_optim=${ATTACKOPTIM}_iter=${ATTACKITER}_slurmid=${SLURM_JOBID} \
--disable_tqdm --workers 4